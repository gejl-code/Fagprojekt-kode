  function [a, f, R, LMlog] = nlss12(ai,tol,fguser,p1,p2,p3,p4,p5,p6, ... 
...
% Revised by Emil Gejl Lauritzen
% Amendment history
% v1.2  2023-03-29 Changes to use Levenberg Marquardt instead of Gauss
% Newton
...
% [f1] = fguser(a1,p1,p2,....);
  ...
  callfg1 = [callfg1,')'];     
%
  mxiter = (5000+ceil(sqrt(n)));
  conv = 0;
  niter = 0;
  err=0;
%
  % Calculate initial guess for eta
    [f0, J] = eval(callfg0);
    nsqrt = norm(f0) / sqrt(n);
    eta = 10^(-10) * norm(f0) / sqrt(n);
    %fprintf('Intial Eta: %f\n', eta);
%
  LMlog = [];
...
%
  % Calculate initial guess for eta
    [f0, J] = eval(callfg0);
    err=err+1;
    multmult = (1/10)^(10-(err-1));
    eta = multmult * norm(f0) / sqrt(n);
    %fprintf('New Eta: %f\n', eta);
    etamult=2;
%
  LMlog = [];
...
%
    if conv ~= 1
%
% Update damping constant
    if sp > 1/3
        eta = eta*(1/etamult);
    elseif sp < 1/4 && ~isnan(sp) && ~isnan(sg)
        eta = eta*etamult;
    end
